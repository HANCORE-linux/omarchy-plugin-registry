...
| `REGISTRY_PREVIOUS_SIGNING_PUBKEY` | Rotation only: the OLD base64 public key. A key swap requires `REGISTRY_ALLOW_KEY_ROTATION=1` **and** this variable matching the on-disk trust root — every surviving signed file (kill list, indexes) keeps verifying fail-closed through the rotation; there is no unverified bypass. Remove both after the first post-rotation regeneration. |
